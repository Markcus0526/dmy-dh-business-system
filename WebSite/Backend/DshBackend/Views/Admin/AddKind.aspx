...
<% var kindinfo = (tbl_kind)ViewData["kindinfo"]; %>
	...
	<div class="col-xs-12">
		<form class="form-horizontal" role="form" id="validation-form">
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right" for="username">商品类型名称：</label>
				<div class="col-sm-9">
                    <div class="clearfix">
					<input type="text" id="kindname" name="kindname" placeholder="请输入名称" class="input-large form-control" <% if (kindinfo != null) { %>value="<%= kindinfo.name %>"<% } %> />
                    </div>
				</div>
			</div>
            
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right" for="videoname">类型图片<%--<span class="red">*</span>--%>：</label>
				<div class="col-sm-9">
                    <div class="clearfix">
                        <img id="previewimg" src="<% if (kindinfo != null && kindinfo.image != null && kindinfo.image != "") { %><%= ViewData["rootUri"] %><%= kindinfo.image %><% } else { %><%= ViewData["rootUri"] %>Content/img/noimage.gif<% } %>" style="max-width:60px;max-height:60px;" />
                        <input type="text" id="image" name="image" value="<% if (kindinfo != null) { %><%= kindinfo.image %><% } %>" style="border:none;width:0;color:White;" readonly />
                        <input type="button" class="btn btn-sm" id="upload_btn" value="选择图片" />
                    </div>
                    <span class="help-inline col-xs-12 col-sm-7">
						<span class="middle">建议大少：60*60px</span>
					</span>
				</div>
            </div>
...
<asp:Content ID="Content3" ContentPlaceHolderID="PageScripts" runat="server">
	...
	<script type="text/javascript">
	    ...
	    var cropw = 60, croph = 60;

	    function redirectToListPage(status) {
	        if (status.indexOf("error") != -1) {
	            $('.loading-btn').button('reset');
	        } else {
	            window.location = rootUri + "Admin/KindList";
	        }
	    }
...
//	    }
	    ...
	    $.validator.addMethod("uniquekindname", function (value, element) {
...
//	        $('#uploadfile').trigger("click");
	    ...
	    function checkKindName() {
	        ...
	        $.ajax({
	            async: false,
	            type: "GET",
	            url: rootUri + "Admin/CheckUniqueKindname",
	            dataType: "json",
	            data: {
	                kindname: kindname,
                    rid: rid
	    ...
	    function checkSelectImage() {
	        if ($("#image").val() != null && $("#image").val().length > 0)
	            return true;
	        else
	            return false;
	    }
	    ...
	    var submitCrop = function (cropfile) {
	        ...
	        $.ajax({
	            ...
	            data: {
	                ...
	                h: $('#crop_h').val(),
	                imgpath: cropfile,
	                kind: "<%= UpImageCategory.KIND %>",
	                size: "<%= CropImageSizes.KIND %>"
	            },
	            type: "POST",
