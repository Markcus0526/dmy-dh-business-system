...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([segue.identifier isEqualToString:@"segueFromGroupBuyToDetail"])
    {
    ...
    else if ([segue.identifier isEqualToString:@"segueFromGroupBuyToSearch"])
    {
...
- (void) initControls
{
    // init right navigation item bar
    [self initNavButtons];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    GoodsTableCell * goodCell = (GoodsTableCell *)[tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if(goodCell == nil)
    {
        goodCell = [[GoodsTableCell alloc]init];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self performSegueWithIdentifier:@"segueFromGroupBuyToDetail" sender:param];
}
...
- (void) callGetGoodsList : (long)kindid pageno:(NSInteger)pageno
{
    ...
    [[CommManager getCommMgr] goodsSvcMgr].delegate = self;
    [[[CommManager getCommMgr] goodsSvcMgr] GetGoodsList:1 kindid:kindid pageno:pageno];    // 1 : 一般
}
...
- (void) onTapNavSearch
{
    ...
    [self performSegueWithIdentifier:@"segueFromGroupBuyToSearch" sender:nil];
}
