...
@interface RegVerifyViewController ()

@end

@implementation RegVerifyViewController
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([segue.identifier isEqualToString:@"segueFromChkVerifyToRegMain"]) {
        RegMainViewController * destCtrl = (RegMainViewController *)segue.destinationViewController;
...
- (void) confirmVerifyKeyResult:(NSInteger)result
{
    if (result == SVCERR_SUCCESS) {
        ...
        // go to next
        [self performSegueWithIdentifier:@"segueFromChkVerifyToRegMain" sender:self];
    }
    else
    {
        NSString * msg = [CommManager getRetMsg:result];
