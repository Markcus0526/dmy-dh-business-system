...
const CGFloat kButtonSpace = 15;
const CGFloat kButtonPadding = 5;
...
- (void)setupButtons {
    ...
    CGFloat yOffset = self.frame.size.height - kButtonSpace - /*kButtonPadding -*/ floorf(kButtonHeight/2);
    ...
    for (UIButton *button in self.buttons) {
        ...
        if (cancelButton == YES) {
            yOffset -= kButtonSpace /*kButtonPadding*/;
            cancelButton = NO;
        }
    }
}
...
- (CGFloat)calculateSheetHeight {
    return ((kButtonHeight * self.buttons.count) + ((self.buttons.count-1) * kButtonPadding) + kButtonSpace*3 /*kButtonHeight/2*/) /*+ 4*/;
}
