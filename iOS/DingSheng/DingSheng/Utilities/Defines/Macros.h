...
//  Macros.h
//
//  Created by Peter van de Put on 03/06/2014.
//
...
// KNLog only in debug mode
#if DEBUG == 1
#define KNLog(...) NSLog(__VA_ARGS__)
#else
#define KNLog(...) do {} while (0)
#endif
...
#define SAFE_BRIDGE_NSSTRING(x)             ((x) ? (__bridge_transfer NSString*)(x) : @"")


#define GOOGLE_V2_MANUAL_TRACKING(screenName) \
        id<GAITracker> tracker = [[GAI sharedInstance] defaultTracker]; \
        [tracker sendView:screenName];

#define GOOGLE_V3_MANUAL_TRACKING(screenName)   \
